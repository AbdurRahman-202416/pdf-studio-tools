...
export interface CompressToTargetViewProps {
  /** Registry slug - drives analytics events. */
  slug: string;
  /** Pre-selected target size when the page loads. */
  defaultTarget: CompressTargetKey;
  title: string;
  subtitle: string;
  badge?: string;
}
...
/**
 * Generic target-size compressor view, shared by every "compress to N" landing
 * page (100KB / 200KB / 500KB / 1MB). All variants hit the same working
 * /tools/compress/target-size endpoint - only the copy and default target differ.
 */
export function CompressToTargetView({
  slug,
  defaultTarget,
  title,
  subtitle,
  badge,
}: CompressToTargetViewProps) {
  const [file, setFile] = useState<File | null>(null);
  const [target, setTarget] = useState<CompressTargetKey>(defaultTarget);
  ...
  const handleSubmit = async () => {
    ...
    setBusy(true);
    trackEvent("tool_started", { tool: slug, target });
    try {
      ...
      trackEvent("tool_failed", { tool: slug });
